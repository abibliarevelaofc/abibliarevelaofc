  ...
  try {
    ...
    if (!resposta.data.items || resposta.data.items.length === 0) {
      return res.status(404).json({
        erro: "Canal do YouTube não encontrado"
      });
    }
...
app.get("/api/videos", async (req, res) => {
  try {
    ...
    if (!canal.data.items || canal.data.items.length === 0) {
      return res.json([]);
    }
...
app.get("/api/comentarios", async (req, res) => {
  try {
    ...
    if (
      ...
      comentarios.data.items.length === 0
    ) {
      return res.json([]);
    }
